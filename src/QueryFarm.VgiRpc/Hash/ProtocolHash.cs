...
/// </para>
/// <para>
/// Not in the preimage: server identity, docstrings, parameter defaults, language-specific
/// type names, and the framework's own request/describe versions. Those vary across processes,
/// builds and ports without changing what is on the wire.
/// </para>
/// <para>
/// Also not in the preimage: whether a stream is an exchange. Not because no port can determine
/// it -- every port can, for most methods -- but because <em>which</em> methods a port can
/// classify depends on how that port's registration works. This port decides
/// producer-vs-exchange from the returned stream, so it cannot state it ahead of the call unless
/// the method carries <see cref="Attributes.StreamKindAttribute"/>. So two ports can disagree
/// about a method while neither is wrong, and a field one port can state and another cannot is
/// not a contract.
/// </para>
/// <para>
/// It still reaches clients as <c>stream_kind</c> on the description, where "unknown" is a
/// sayable answer. A hash has no such option -- which is the whole difference: a description may
/// admit what it does not know, a fingerprint may not.
/// </para>
/// </remarks>
